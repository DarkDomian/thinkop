...
type FormStepProps = {
  onComplete: (isComplite: boolean) => void;
};

const FirstStep: React.FC<FormStepProps> = ({ onComplete }) => {
  ...
  return (
    <>
      <div className="flex md:flex-row flex-col md:justify-between md:my-24 my-5 text-center md:text-left">
        ...
        <Button className="" onClick={handleClearForm}>
          Отменить заполнение
        </Button>
      </div>
      <Form
      ...
      >
        ...
        >
          <Select placeholder="Формат">
            <Select.Option value="Онлайн-платформа">
              Онлайн-платформа
            </Select.Option>
        ...
        <Form.Item label="№УНФ или отсутствует" name="unfNumber" className="">
          <InputNumber
            controls={false}
            className="w-full"
            formatter={(value) => {
        ...
        <Form.Item
          ...
          name="estimatedCost"
          className=""
          rules={[{ type: "number", message: "Ожидается число" }]}
        >
        ...
        <Form.Item label="Синапсис" name="sinapsis" className="row-span-2">
          <Input.TextArea
